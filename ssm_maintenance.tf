...
resource "aws_ssm_maintenance_window" "custom" {
  count    = (var.default_patch_groups ? 0 : 1) * length(var.maintenance_windows)
  name     = "${var.client_name}-Maintenance-Window-Patch-${var.maintenance_windows[count.index]}"
  schedule = var.schedule_windows[count.index]
  duration = 3
  cutoff   = 1
...
resource "aws_ssm_maintenance_window_task" "task_install_patches" {
  ...
  task_invocation_parameters {
    run_command_parameters {
      ...
      notification_config {
        notification_arn    = aws_sns_topic.patching_alerts.arn
        notification_events = ["Failed","Cancelled","TimedOut"]
...
resource "aws_ssm_maintenance_window_task" "task_install_patches_custom" {
  count            = (var.default_patch_groups ? 0 : 1) * length(var.maintenance_windows)
  name             = "${var.client_name}-Maintenance-Window-Patch-${var.maintenance_windows[count.index]}"
  window_id        = aws_ssm_maintenance_window.custom[count.index].id
  task_type        = "RUN_COMMAND"
  task_arn         = "AWS-RunPatchBaseline"
  service_role_arn = aws_iam_role.ssm_maintenance.arn
  priority         = var.task_install_priority
  ...
  targets {
    key    = "WindowTargetIds"
    values = aws_ssm_maintenance_window_target.target_install_custom.*.id
  }
...
resource "aws_ssm_maintenance_window_target" "target_install_custom" {
  count         = (var.default_patch_groups ? 0 : 1) * length(var.maintenance_windows)
  name          = var.maintenance_windows[count.index]
  window_id     = aws_ssm_maintenance_window.custom[count.index].id
  resource_type = "INSTANCE"
